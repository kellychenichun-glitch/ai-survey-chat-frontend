...
interface StatsData { surveyId?: string|number; totalResponses: number; completionRate: number; avgDurationSeconds: number; questions: QuestionStats[] }
interface Survey { id?: string|number; title: string; description?: string; status?: string; questions: any[]; settings?: any; }
  ...
  return (
    <div>
      <div className="flex items-end gap-6 mb-4">
        ...
        <div className="flex gap-1 pb-2">
          {Array.from({length:scale},(_,i) => (
            <Star key={i} className={`h-6 w-6 ${i < Math.round(stats.average||0) ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}`}/>
          ))}
        </div>
        {stats.nps != null && (
      ...
      <div className="space-y-1.5">
        {Array.from({length:scale},(_,i) => { const n=i+1; const c=stats.distribution?.[n]||0; const w=pct(c,stats.totalAnswers||1); return (
          <div key={n} className="flex items-center gap-3 text-sm">
            <span className="w-4 text-right text-gray-500">{n}</span>
            <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden"><div className="h-full bg-amber-400 rounded-full" style={{width:`${w}%`}}/></div>
            <span className="w-8 text-right text-gray-400 tabular-nums">{c}</span>
          </div>
        )})}
      </div>
    </div>
...
function TextAnswers({ answers }: { answers:any[] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {answers.length > 3 && (
        <button onClick={() => setExpanded(!expanded)} className="text-sm text-blue-600 hover:underline flex items-center gap-1">
          {expanded ? <><ChevronUp className="h-3.5 w-3.5"/>收起</> : <><ChevronDown className="h-3.5 w-3.5"/>查看全部 {answers.length} 份回答</>}
        </button>
      )}
    </div>
  )
...
function TrendChart({ surveyId }: { surveyId: string|number }) {
  ...
  if (loading) return <div className="h-24 flex items-center justify-center"><RefreshCw className="h-5 w-5 animate-spin text-gray-300"/></div>
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-1">{q.type?.replace('_',' ')}</p>
      ...
      {(q.type==='single_choice'||q.type==='multi_choice') && s.optionCounts &&
        <ChoiceChart options={q.options||[]} counts={s.optionCounts} total={s.totalAnswers||0}/>}
      {q.type==='rating' && s.distribution &&
        <RatingDisplay stats={s} scale={q.scale||5}/>}
      {(q.type==='text_short'||q.type==='text_long') && s.answers &&
        <TextAnswers answers={s.answers}/>}
      {q.type==='ranking' && s.averageRanks && (
        <div className="space-y-2">
          {(q.items||[]).sort((a:any,b:any) => (s.averageRanks[a.id]||99)-(s.averageRanks[b.id]||99)).map((item:any,i:number) => (
            <div key={item.id} className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400">平均 {s.averageRanks[item.id]?.toFixed(1)}</span>
            </div>
          ))}
        </div>
      )}
      {q.type==='matrix' && s.cellCounts && (
        <div className="overflow-x-auto">
          <table className="text-sm w-full">
            <thead><tr>
              <th className="text-left py-1 pr-3 text-gray-500 font-medium min-w-[100px]"></th>
              {(q.columns||[]).map((c:any) => <th key={c.id} className="text-center px-2 py-1 text-gray-600 font-medium">{c.text}</th>)}
            </tr></thead>
            <tbody>{(q.rows||[]).map((r:any) => (
              <tr key={r.id} className="border-t border-gray-100">
                <td className="py-2 pr-3 text-gray-700">{r.text}</td>
                {(q.columns||[]).map((c:any) => {
                  const count = s.cellCounts[r.id]?.[c.id]||0
                  return <td key={c.id} className="text-center px-2 py-2">{count>0 ? <span className="bg-blue-100 text-blue-700 rounded px-1.5">{count}</span> : <span className="text-gray-300">0</span>}</td>
                })}
              </tr>
            ))}</tbody>
          </table>
  ...
  const run = async () => {
    ...
    try {
      const summary = {
        ...
        avgDuration: fmtDur(stats.avgDurationSeconds),
        questions: stats.questions.map(qs => ({ title:qs.question.title, type:qs.question.type, stats:qs.stats }))
      }
      const res = await fetch(`${API}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: `你是專業問卷數據分析師，請用繁體中文分析以下問卷結果：\n1. 整體概況（2-3句）\n2. 主要發現（3-5個重點）\n3. 值得關注的問題\n4. 具體改善建議（3-5條）\n\n問卷數據：${JSON.stringify(summary,null,2)}\n\n請用清晰結構回覆，使用 emoji 讓報告易讀。` })
      })
      const data = await res.json()
      setAnalysis(data.message || '分析失敗')
    ...
    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl border border-purple-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-purple-500 bg-purple-100 px-2 py-0.5 rounded-full">Claude Powered</span>
        </div>
...
export default function StatsDashboard({ survey, stats }: Props) {
  ...
  const surveyId = survey.id as string|number

  const refresh = async () => {
    try {
      const res = await fetch(`${API}/api/v1/surveys/${surveyId}/stats`, { cache: 'no-store' })
  ...
  const handleExport = async () => {
    setExporting(true)
    try {
      const res = await fetch(`${API}/api/v1/surveys/${surveyId}/responses/export`)
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href=url; a.download=`survey_${surveyId}.csv`; a.click()
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b shadow-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-semibold text-gray-900 truncate max-w-sm">{survey.title}</h1>
              <p className="text-xs text-gray-400">問卷統計</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link href={`/admin/surveys/${surveyId}/responses`} className="flex items-center gap-1.5 text-sm border px-3 py-2 rounded-lg text-gray-700 hover:bg-gray-50 transition">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {cards.map(item => (
            <div key={item.label} className="bg-white rounded-xl border shadow-sm p-5">
        ...
        {currentStats.totalResponses > 0 && (
          <div className="bg-white rounded-xl border shadow-sm p-6">
            <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <TrendingUp className="h-4 w-4 text-blue-500"/>回覆趨勢（近 30 天）
            </h3>
            <TrendChart surveyId={surveyId}/>
          </div>
        )}
        ...
        {currentStats.totalResponses === 0 ? (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-lg font-medium">還沒有回覆</p>
            <p className="text-sm mt-1">發布問卷後開始收集資料</p>
          </div>
        ) : (
