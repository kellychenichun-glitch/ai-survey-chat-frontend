...
import { ArrowLeft, Download, Users, Clock, TrendingUp, BarChart2, Sparkles, Star, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react'
...
function ChoiceChart({ options, counts, total }: { options: {id:string;text:string}[]; counts: Record<string,number>; total: number }) {
  return (
    <div className="space-y-3">
      {options.map(o => {
        const c = counts[o.id] || 0; const w = pct(c, total)
        return (
          <div key={o.id}>
            <div className="flex justify-between text-sm mb-1">
              <span className="text-gray-700">{o.text}</span>
              <span className="text-gray-500 font-medium tabular-nums">{c} ({w}%)</span>
            </div>
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{width:`${w}%`}} />
            </div>
          </div>
        )
      })}
...
export default function StatsDashboard({survey,stats}:Props) {
  ...
  const cards=[{icon:Users,label:'總回覆數',value:currentStats.totalResponses,color:'text-blue-600'},{icon:TrendingUp,label:'完成率',value:`${Math.round(currentStats.completionRate*100)}%`,color:'text-green-600'},{icon:Clock,label:'平均填答時間',value:fmtDur(currentStats.avgDurationSeconds),color:'text-amber-600'},{icon:BarChart2,label:'題目數',value:currentStats.questions.length,color:'text-purple-600'}]
  return(
