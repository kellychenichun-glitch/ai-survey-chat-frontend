...
const QTYPES:{type:QuestionType;emoji:string}[]=[{type:'single_choice',emoji:'🔘'},{type:'multi_choice',emoji:'☑️'},{type:'text_short',emoji:'✏️'},{type:'text_long',emoji:'📝'},{type:'rating',emoji:'⭐'},{type:'matrix',emoji:'📊'},{type:'ranking',emoji:'🏆'}]
...
export default function SurveyBuilder({mode,initialSurvey}:Props) {
  ...
  const handleSave=async()=>{if(!survey.title.trim()){setError('請填寫問卷標題');return}setSaving(true);setError('');try{if(mode==='create'){const c=await createSurvey(survey);router.push(`/admin/surveys/${c.id}/edit`)}else{await updateSurvey(String(survey.id),survey)}}catch(e:any){setError(e.message)}finally{setSaving(false)}}
  const handlePublish=async()=>{if(!survey.title.trim()){setError('請填寫問卷標題');return}if(survey.questions.length===0){setError('請至少新增一個題目');return}setPublishing(true);setError('');try{let id=survey.id;if(mode==='create'||!id){const c=await createSurvey(survey);id=c.id}else{await updateSurvey(String(id),survey)}await publishSurvey(String(id));router.push('/admin/surveys')}catch(e:any){setError(e.message)}finally{setPublishing(false)}}
  const handleAi=async()=>{if(!aiTopic.trim())return;setAiLoading(true);setError('');try{const r=await aiGenerateSurvey({topic:aiTopic,targetCount:8,language:'zh-TW'});setSurvey(s=>({...s,title:r.title||s.title,description:r.description||s.description,questions:r.questions||s.questions}));setShowAi(false);setAiTopic('')}catch(e:any){setError(e.message)}finally{setAiLoading(false)}}
  return(
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-5">
        ...
        {showAi&&(
          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200 p-5">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleAi} disabled={aiLoading||!aiTopic.trim()} className="px-4 py-2 rounded-lg bg-purple-600 text-white text-sm hover:bg-purple-700 disabled:opacity-50 flex items-center gap-1.5 transition">{aiLoading?<RefreshCw className="h-4 w-4 animate-spin"/>:<Sparkles className="h-4 w-4"/>}{aiLoading?'生成中...':'生成'}</button>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3">{survey.questions.map((q,idx)=>(<QuestionEditor key={q.id} q={q} idx={idx} total={survey.questions.length} expanded={expandedQ===q.id} onToggle={()=>setExpandedQ(expandedQ===q.id?null:q.id)} onUpdate={p=>updateQ(q.id,p)} onRemove={()=>removeQ(q.id)} onMove={dir=>moveQ(idx,dir)}/>))}</div>
        <div className="bg-white rounded-xl border shadow-sm p-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">新增題目</h3>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-2">{QTYPES.map(({type,emoji})=>(<button key={type} onClick={()=>addQ(type)} className="flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 border-dashed hover:border-blue-400 hover:bg-blue-50 transition text-xs text-gray-500 hover:text-blue-700"><span className="text-lg">{emoji}</span>{QUESTION_TYPE_LABELS[type]}</button>))}</div>
        </div>
...
function QuestionEditor({q,idx,total,expanded,onToggle,onUpdate,onRemove,onMove}:{q:Question;idx:number;total:number;expanded:boolean;onToggle:()=>void;onUpdate:(p:Partial<Question>)=>void;onRemove:()=>void;onMove:(d:-1|1)=>void}) {
    ...
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
      ...
      {expanded&&(
        <div className="border-t bg-gray-50 px-4 py-4 space-y-4">
          ...
          <div><label className="text-xs font-medium text-gray-600 mb-1 block">說明（選填）</label><input value={(q as any).description||''} onChange={e=>onUpdate({description:e.target.value}as any)} placeholder="補充說明" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white"/></div>
          ...
          {(q.type==='single_choice'||q.type==='multi_choice')&&(
            <div className="space-y-2">
              ...
              {((q as any).options||[]).map((opt:any,oi:number)=>(<div key={opt.id} className="flex gap-2 items-center"><div className={`w-4 h-4 flex-shrink-0 border-2 border-gray-300 ${q.type==='single_choice'?'rounded-full':'rounded'}`}/><input value={opt.text} onChange={e=>updOpt(oi,e.target.value)} placeholder={`選項 ${oi+1}`} className="flex-1 rounded-lg border px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white"/><button onClick={()=>delOpt(oi)} disabled={((q as any).options||[]).length<=1} className="text-red-400 hover:text-red-600 disabled:opacity-20 flex-shrink-0"><Trash2 className="h-3.5 w-3.5"/></button></div>))}
