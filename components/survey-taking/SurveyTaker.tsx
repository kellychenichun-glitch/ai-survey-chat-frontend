...
export default function SurveyTaker({ survey }: Props) {
  ...
  const canNext = useCallback(() => {
    if (!q) return false; if (!q.required) return true; const a = answers[q.id]
    ...
    if (q.type === 'matrix') { const rows = (q as any).rows || []; return rows.every((r: any) => a?.rows?.[r.id] != null) }
    return true
  }, [q, answers])
...
function QuestionInput({ q, value, onChange }: { q: Question; value: any; onChange: (v: any) => void }) {
  ...
  if (q.type === 'text_long') return <textarea value={value?.text||''} onChange={e=>onChange({text:e.target.value})} placeholder={(q as any).placeholder||'請詳細描述...'} rows={5} className="w-full rounded-xl border-2 px-4 py-3 text-sm focus:outline-none focus:border-blue-500 transition resize-none"/>
  if (q.type === 'rating') {
    const scale=(q as any).scale||5; const style=(q as any).style||'stars'; const val=value?.value
    if (style==='stars') return (<div><div className="flex gap-2">{Array.from({length:scale},(_,i)=>i+1).map(n=>(<button key={n} onClick={()=>onChange({value:n})} className="transition hover:scale-110"><Star className={`h-9 w-9 transition ${val>=n?'fill-amber-400 text-amber-400':'text-gray-200 hover:text-amber-300'}`}/></button>))}</div></div>)
