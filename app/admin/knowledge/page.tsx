...
import { ArrowLeft, Plus, Trash2, BookOpen, Edit2, Check, X, RefreshCw, Database } from 'lucide-react'
      ...
      <header className="bg-white border-b shadow-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-blue-600"/><h1 className="font-semibold text-gray-900">知識庫管理</h1></div>
          </div>
          <button onClick={() => setCreating(true)} className="flex items-center gap-1.5 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition"><Plus className="h-4 w-4"/>新增知識庫</button>
        </div>
      </header>
        ...
        {creating && (
          <div className="bg-white rounded-xl border shadow-sm p-6 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={()=>{setCreating(false);setNewName('');setNewDesc('')}} className="flex items-center gap-1.5 border px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50"><X className="h-4 w-4"/>取消</button>
            </div>
          </div>
        )}
        {loading ? (
          ...
          <div key={kb.id} className="bg-white rounded-xl border shadow-sm p-5 hover:shadow-md transition">
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2 ml-4">
                <Link href={`/admin/knowledge/${kb.id}`} className="flex items-center gap-1.5 text-sm border px-3 py-1.5 rounded-lg text-gray-700 hover:bg-gray-50 transition"><Edit2 className="h-3.5 w-3.5"/>管理條目</Link>
                <button onClick={()=>handleDelete(kb.id, kb.name)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><Trash2 className="h-4 w-4"/></button>
              </div>
            </div>
          </div>
        ))}
