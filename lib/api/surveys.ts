...
export async function listSurveys(): Promise<Survey[]> {
  const res = await fetch(`${API}/api/surveys`, { headers: authHeaders() });
  const d = await handle<any>(res);
  return Array.isArray(d) ? d : d.surveys || [];
}

export async function getSurvey(id: string): Promise<Survey> {
  const res = await fetch(`${API}/api/surveys/${id}`, { headers: authHeaders() });
  const d = await handle<any>(res);
  return d.survey || d;
}
...
export async function createSurvey(survey: Survey): Promise<Survey> {
  const res = await fetch(`${API}/api/surveys`, {
    method: 'POST', headers: authHeaders(), body: JSON.stringify(survey),
  });
  const d = await handle<any>(res);
  return d.survey || d;
}
...
export async function updateSurvey(id: string, survey: Partial<Survey>): Promise<Survey> {
  const res = await fetch(`${API}/api/surveys/${id}`, {
    method: 'PUT', headers: authHeaders(), body: JSON.stringify(survey),
  });
  const d = await handle<any>(res);
  return d.survey || d;
}
...
export async function aiGenerateSurvey(req: {
  topic: string;
  targetCount?: number;
  language?: string;
  context?: string;
  ...
  const res = await fetch(`${API}/api/surveys/ai-generate`, {
